...
def find_optimal_nd_iteratively():
    ...
    file_prev_tsv_results = f"../../data/{CASE_NAME}_result_tsv.yaml"
    ...
    z_mask_target_values = get_z_mask_flat_target(frequency_points, target_ohm_value)
    print(f"Using flat target impedance: {target_ohm_value:.4f} Ohm for iterative search.")
    ...
    if optimal_nd_result:
        ...
        # --- 输出 YAML ---
        file_output_ga_dtc = f"../data/{CASE_NAME}_result_dtc_ga_OptimalNd{final_nd}.yaml"
        result_yaml_output = {
            "dtcs": [], 
            "tsvs": [], 
